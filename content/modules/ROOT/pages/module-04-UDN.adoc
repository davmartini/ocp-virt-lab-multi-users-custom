...
+
. Examinons maintenant notre UDN
+
...
. Nous pouvons notamment observer :
    ...
    * L'utilisation de l'IPAM, qui garantit que les adresses IP des VMs seront persistantes
---
...
Nous avons désormais deux VMs dans notre namespace *udn-project-X*, `vm-web` et `vm-db`, que nous allons utiliser pour vérifier la *persistance des IP* sur un UDN puis pour tester les *Network Policies*.
Démarrons par la vérification de la persistance des IP.

. Dans la liste *Virtualization* -> *VirtualMachines*, localisez votre VM `vm-db`
. Notez l'adresse IP affichée dans la partie Network de l'Overview. C'est une IP qui a été dédiée à cette machine par l'IPAM de notre UDN.
...
. Une fois la VM revenue à l'état *Running*, vérifiez à nouveau l'adresse IP affichée dans la partie Network de l'Overview.
. L'adresse IP de la VM n'a pas changée. Cette IP est liée à cette VM et ne changera pas malgré les lives migrations ou les redémarrages.
+
image::2025_spring/module-09-networking/48.png[link=self, window=blank, width=100%]
+
