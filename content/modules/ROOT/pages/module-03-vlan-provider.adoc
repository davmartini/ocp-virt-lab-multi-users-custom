...
== Vérifier la configuration des noeuds physiques

Nous vous avons donné les droits pour que vous puissiez inspecter la configuration des noeuds physiques directement depuis la console *OpenSHift*

. Pour cela, il vous suffit d'aller dans le menu à gauche *Networking* -> *Node Network Configuration*.
+
image::2025_spring/module-09-networking/nnc01.png[link=self, window=blank, width=75%]

. Vous pourrez voir en temps réel l'ensemble des noeuds physiques sur lesquels vous pouvez zoomer ou bouger les objets simplement.
+
image::2025_spring/module-09-networking/nnc02.png[link=self, window=blank, width=75%]
+
image::2025_spring/module-09-networking/nnc03.png[link=self, window=blank, width=75%]
...
+
. Maintenant nous allons effectuer un clic droit sur la machine `fedora-bridged-1`et cliquer sur clone.
+
...
. Une fois la VM revenue à l'état *Running*, vérifiez à nouveau les adresses IP affichées dans la partie Network de l'Overview.
