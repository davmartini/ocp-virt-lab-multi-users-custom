...
En tant qu'utilisateur, vous avec les permissions nécessaires pour gérer votre projet OpenShift : vous pouvez arrêter (*stop*), démarrer (*start*), redémarrer (*restart*), mettre en pause (*pause*), et sortir de pause (*unpause*) les machines virtuelles depuis la console web.
...
[[live_migrate]]
== Effectuer une VM Live Migration
...
Étant simple utilisateur OpenShift, vous ne verrez pas le noeud sous jacent hébergeant votre VM. OpenShift étant une technologie cloud, l'infrastructure doit être transparente pour un utilisateur final. 
...
. Vous pouvez également voir l'ensemble des migrations de vos projects en allant dans le menu à gauche *Overview* puis dans l'onglet *Migration* en étant bien sûr d'être dans votre projet.
+
image::2025_spring/module-01-intro/34_Migration_Overview.png[link=self, window=blank, width=100%]


[[storage_live_migrate]]
== Effectuer un Storage Live Migration

Dans cette section, nous allons migrer le disque d'une VM d'une class de stockage à une autre sans arrêter la VM. Une class de stockage peut correspondre à un type de stockage ou a une QoS particulière ou simplement à un founisseur de stockage particulier.

. Sélectionnez votre VM et allez dans le menu *Actions* -> *Migration* -> *Storage*.
+
image::2025_spring/module-01-intro/35_Migration_Storage.png[link=self, window=blank, width=100%]

. Vous pouvez choisir de migrer l'ensemble des disques d'une VM ou un seul disque en particulier.
+
image::2025_spring/module-01-intro/36_Migration_Storage.png[link=self, window=blank, width=100%]

. Vous devez choisir vers quelle classe de stockage vous souhaitez migrer votre disque.
+
image::2025_spring/module-01-intro/37_Migration_Storage.png[link=self, window=blank, width=100%]

. Une fois que vous avez fait votre choix, vous pouvez lancer la migration.
+
image::2025_spring/module-01-intro/38_Migration_Storage.png[link=self, window=blank, width=100%]

. Une fois lancée, vous pouvez suivre la migration si vous le souhaitez.
+
